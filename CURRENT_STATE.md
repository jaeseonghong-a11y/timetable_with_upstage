...
1. 시작 즉시 `git status --short`와 `git log --oneline -5`를 읽는다. GitHub에 push된 마지막
   커밋은 `77e924c`(`origin/main`과 동기화 확인됨). **최우선 확인 사항**:
   a. 이번 세션에서 만든 UI 변경(수강내역/졸업요건 검토 화면)을 실제 브라우저에서 확인한다
      — 아직 아무도 시각적으로 검증하지 않았다.
   b. 사용자에게 Vercel의 안 쓰는 `GEMINI_API_KEY` 환경변수를 지워도 되는지 물어본다.
   c. GitHub PR #1(`YOUNSUHPARK-patch-1`)이 닫혔는지, 윤서 조원이 GA4 맞춤 정의를
      등록했는지 확인한다(여러 세션째 미확인).
   d. 정현 조원이 `YJH-1023/h`에서 더 진행한 게 있는지 궁금하면
      `git remote add jeonghyeon https://github.com/YJH-1023/h.git && git fetch jeonghyeon`
      후 blob SHA 대조 방식을 재사용한다(지난 세션 대화 로그 참조).
...
5. [중간점검 후 검토] 교양과목 캐싱은 인메모리 TTL로 시작했다(`web/src/lib/cache-store.ts`).
   ...
   "낮음 (나중)" 참조.
6. AI 필러 후보 수 축소(현재 8개 중 5개)는 계속 보류 중 — 10000개 한도 문제가 다시 보고되면
   고려한다.
