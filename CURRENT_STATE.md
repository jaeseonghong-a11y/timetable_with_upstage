...
- ✅ (해결) 분반 목록 직접 선택 변경 + 사용자 요청 7종(강의형식 필터·동적 공강일 필터·학사문서
  카드 압축·첨부 경로 안내·체크상태 버그·조합 버전/추가과목/총학점·최대학점 21)을 구현·검증하고
  **커밋(`8b50ffc`)·Vercel 프로덕션 배포까지 완료**했다(사용자가 명시적으로 지시).
  배포 URL: https://timetable-with-upstage.vercel.app (alias), 배포ID `dpl_6RQXa8mDqAo1UV2JpBq9GofaFUHz`.
  Vercel 프로젝트는 저장소 **루트**에 연결돼 있음 — `web/` 안에서 vercel 명령을 실행하면
  `web/web` 경로 오류가 나니 항상 저장소 루트에서 실행할 것.
- ✅ (해결) **후속 변경 3건 — 커밋(`de0b7cb`)·배포(`dpl_EygxgmjnRLvdor4QdtATQW4X1X6G`) 완료**
  (위 배포 이후 사용자가 추가 요청):
  1. 과목을 처음 담을 때 기본 목적지를 "선택 그룹 1" → "필수 과목"으로 변경
  2. 학점 범위 입력 화살표 step을 0.5 → 1로 변경
  3. 강의 형식 필터를 "생성된 시간표 결과 필터링"에서 "과목을 고르는 카탈로그 단계 필터링"으로
     이동 — `formatFilteredCourseGroups`가 `disabledCourseTypes`로 후보 자체를 걸러내고,
     `result` useMemo는 더 이상 형식 필터를 직접 알지 못함(간접적으로만 반영됨). 이미 선택된
     교양 그룹은 `electiveCourseGroups`/`electivePreviewGroups` state에 원본(미필터) 그대로
     저장하고, 화면에는 `filterGroupCandidatesByFormat()`로 그때그때 필터링한 사본만 보여준다
     (필터 체크 해제→재선택 시 데이터 유실 방지).
  - `npm run lint/typecheck/test(87개)/build` 전부 통과 확인. 다음 도구는 커밋·배포 여부를
    사용자에게 확인부터 받을 것(하네스가 "커밋 없는 프로덕션 배포"를 이미 한 번 막았음).
- ✅ (해결, 커밋 `16d450c`, 배포 `dpl_3HFo8PzWbmkf3Ywm28RLoa2AeMHd`) **버그: 학사문서 분석이
  "Solar 추출 결과가 학사 데이터 형식을 만족하지 않았습니다"로 간헐적 실패** — 사용자가 프로덕션에서 재현, 같은 파일을 재시도했을 때만 발생(항상은 아님)이라고
  보고. 원인 진단을 위해 실제 Solar API(`solar-pro3`)로 45행짜리 합성(가짜) 수강과목 표를
  만들어 동일 프롬프트로 2회 라이브 호출: 1회는 성공(45/45 파싱), 1회는 `finish_reason:"stop"`인데도
  `reviewReasons` 필드에 중첩 배열(`[[]]`)이 섞인 형식 오류 JSON을 반환해 파싱 실패 — **Solar
  자체의 비결정적 출력(같은 입력에도 가끔 형식이 깨짐)이 원인이지, 이번 세션 코드 변경 때문이
  아님**. `max_tokens` 조정으로는 해결 안 됨(오히려 다른 실패 패턴이 나옴 — 토큰 한도 문제가
  아니라 순수 변동성). `web/src/lib/academic-document.ts`의 `extractAcademicProfile`에
  `requestAndParseWithRetry`를 추가해, 첫 Solar 응답이 `AcademicExtractionError`(JSON 계약
  위반)를 던지면 **동일 프롬프트로 1회 자동 재시도**하도록 수정(기존에 있던 "누락 과목코드
  재시도" 로직과 같은 패턴). 회귀 테스트(`route.test.ts`)도 재시도 1회까지 실패하는 경우로
  갱신. `npm run lint/typecheck/test(87개)/build` 전부 통과.
- ✅ (해결) **버그: 강의형식 필터를 바꾸면 이미 선택한 과목이 선택 해제됨** — 사용자가 오프라인
  과목을 선택한 뒤 다른 과목을 찾으려고 "오프라인" 필터 체크를 해제하면, 이미 선택했던 오프라인
  과목이 계획에서 사라지는 버그를 보고받아 수정. 원인: 지난 커밋에서 카탈로그 필터링용
  `formatFilteredCourseGroups`를 "선택된 과목" 계산(`effectiveSelectedGroupIds`/
  `selectedCourseGroups`)에도 그대로 재사용했는데, 이 필터는 후보 자체를 걸러내므로 이미 선택한
  과목의 유일한 분반이 필터에 걸리면 그룹 전체가 통째로 빠져나가 "선택 해제"처럼 보였음.
  수정: "선택된 과목"/결과 계산은 다시 `availableCourseGroups`(기수강 제외만, 형식 필터 미적용)를
  쓰도록 되돌리고, 카탈로그 브라우징 목록(`visibleCourseGroups` 등)만 `formatFilteredCourseGroups`를
  쓰게 분리. 즉 강의형식 필터는 "새로 고를 때 카탈로그에서 뭘 보여줄지"에만 영향을 주고, 이미
  계획에 넣은 과목은 필터를 바꿔도 그대로 유지된다. `npm run lint/typecheck/test(87개)/build`
  전부 통과.
...
1. 후속 변경 3건(기본 목적지 필수/학점 step 1/강의형식 필터 위치 이동)의 커밋·배포 여부를
   사용자에게 확인한다. 구현·검증(lint/typecheck/test 87개/build)은 이미 끝났다. 커밋 전 항상
   `git status --short`로 다른 unstaged 변경(START_HERE.md, docs/02 등)과 섞이지 않는지 확인할 것.
   배포는 저장소 **루트**에서 `npx vercel deploy --prod --yes` (web/ 안에서 실행하면 경로 오류).
