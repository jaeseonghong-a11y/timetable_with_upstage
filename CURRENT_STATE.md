...
(최신 배포 `dpl_EL8XBkNexBGNCwiNaa2zvEN5UxUx`). 강의 형식 필터·동적 공강일 필터·학사문서
카드 압축·Solar 추출 재시도·필터-선택 상태 분리 버그 수정까지 전부 반영된 상태다.
**단, 아래 "AI 시간표 추천 + 교양과목 캐싱" 섹션의 작업은 로컬에만 있고 아직 배포 전이다**
(품질 게이트·라이브 검증 완료, 사용자 커밋/배포 확인 대기 중).
...
- `npm run dev`(백그라운드) → `curl`로 `/api/skku-electives`(캐시 전/후 응답시간 비교),
  ...
  렌더 확인) 직접 호출.
- 아직 `git add`/`git commit`/배포 없음 — 사용자 확인 대기 중.
...
1. 시작 즉시 `git status --short`를 읽는다. 이 인수인계 시점 기준으로는 위 "AI 시간표 추천 +
   교양과목 캐싱" 섹션에 나열된 신규/수정 파일이 아직 미커밋 상태로 남아있어야 한다(품질
   게이트는 전부 통과, 라이브 검증도 완료했지만 커밋·배포는 사용자 확인 대기 중이었음).
