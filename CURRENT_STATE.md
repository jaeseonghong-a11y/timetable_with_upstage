...
1. 시작 즉시 `git status --short`와 `git log --oneline -5`를 읽는다. GitHub에 push된 마지막
   커밋은 `32edf3b`(`origin/main`과 동기화 확인됨). **최우선 확인 사항**:
   a. 사용자가 GA4 새 측정 ID를 전달했는지 확인 — 받았다면
      `web/src/lib/analytics-config.ts`의 `GA_MEASUREMENT_ID` 교체 + 배포.
   b. 사용자에게 `GEMINI_API_KEY` Vercel 환경변수를 직접 지웠는지 확인(하네스가 차단해서
      내가 못 지움).
   c. 이번 세션(및 이전 세션)에서 만든 UI 변경들을 실제 브라우저에서 확인한다 — 여러
      세션째 아무도 시각적으로 검증하지 않았다. 최우선으로 권장.
   d. "친구와 시간표 맞추기" 기능 진행 여부를 사용자에게 물어본다(방 코드 + Vercel KV
      기반으로 설계 논의는 끝나있음, 위 6번 참고).
2. 규나 조원이 졸업요건 화면을 다시 만들다가 "라벨 기반 scope 분류 개선"(대학공통→기타
   통합, DS/전공/교양 라벨 패턴 매칭)을 다시 요청하면, `web/src/lib/academic-document.ts`의
   `normalizeRequirementScope`를 기준으로 새로 구현한다(그녀의 옛 커밋 `058d24e`를 그대로
   못 쓰는 이유는 위 5번 참고).
3. 공식 루브릭(`docs/08`) 대비 데모 증거(파이프라인 도식, LLM 비교, Upstage 적용 전후 수치)를
   준비한다. `docs/05_미해결_과제.md` P16 참조. 데모데이(2026-07-25)가 가까워지고 있다.
4. [중간점검 후 검토] 교양과목 캐싱은 인메모리 TTL로 시작했다(`web/src/lib/cache-store.ts`).
