...
1. 시작 즉시 `git status --short`와 `git fetch origin && git branch -a`를 읽는다. GitHub에
   push된 마지막 커밋은 `c5fc61c`(`origin/main`과 동기화 확인됨). **최우선 확인 사항**:
   ...
   b. 윤서 조원이 GA4 맞춤 정의(측정기준: `doc_type`/`error_type`/`weight_type`/`field_name`/
...
2. 멘토 P1-6 "재미난 기능 1~2개"(교수 제외/친구 시간표 비교/공강 확보/특정 요일 제외) 중
   하나를 사용자와 상의해 구현한다 — 사용자가 이번 세션엔 명시적으로 보류시켰을 뿐 필요성
   자체는 인정한 상태.
3. 졸업요건의 남은 영역을 교양 탐색 기본 영역과 연결하고, 캠퍼스 이동/온라인 수업 예외 규칙을
   ...
   방식(1단계)까지 구현 완료, 방 코드 방식(2단계)은 사용자가 명시적으로 후순위 연기함. 이름
   있는 고정 일정(알바 등)도 이번 세션에 구현 완료.
