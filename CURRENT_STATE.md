...
- ✅ (해결) 분반 목록 직접 선택 변경 + 사용자 요청 7종(강의형식 필터·동적 공강일 필터·학사문서
  카드 압축·첨부 경로 안내·체크상태 버그·조합 버전/추가과목/총학점·최대학점 21)을 전부 구현·
  검증했다. 커밋은 사용자가 명시적으로 지시한 뒤 진행 예정(아래 참조).
- ⚠️ **Vercel 배포 미완료**: `vercel deploy --prod --yes`가 하네스 안전장치에 막혔다(커밋 전
  프로덕션 배포 시도 + 대량 미검토 변경 35개 unstaged 파일 포함이 이유). 커밋 완료 후 사용자에게
  다시 배포 확인을 받고 `npx vercel deploy --prod --yes`를 저장소 루트에서 실행할 것
  (`.vercel/project.json`에 `timetable-with-upstage` 프로젝트가 이미 연결돼 있음 — 루트 디렉터리
  설정이 `web/`이므로 반드시 저장소 루트에서 vercel 명령을 실행해야 함, `web/` 안에서 실행하면
  `web/web` 경로 오류가 남).
...
1. 사용자가 요청한 커밋을 진행한다(이 세션에서 아직 안 함 — 사용자가 명시적으로 지시했으므로
   다음 턴에 실행). 커밋 후 `npx vercel deploy --prod --yes`를 **저장소 루트**에서 실행해 배포한다
   (사용자에게 배포 확인을 받은 뒤 진행 — 하네스 안전장치가 순서를 지키지 않은 배포를 막았었음).
