...
- Vercel 배포 준비 → Function 고정 payload 4.5MB 한도에 multipart overhead가 남도록 학사문서·
  ...
  재확인했다. 실제 Vercel 프로젝트 가져오기·환경변수 등록·배포는 사용자 계정에서 마무리한다.
...
1. Vercel에서 GitHub 저장소를 import하고 Root Directory를 `web`으로 지정한 다음, Production·Preview에
   `UPSTAGE_API_KEY`를 Sensitive 환경변수로 등록해 배포한다. `NEXT_PUBLIC_` 접두사는 절대 붙이지 않는다.
