...
1. 시작 즉시 `git status --short`와 `git log --oneline -5`를 읽는다. **최우선 확인 사항**:
   a. **브라우저로 직접 열어서 클릭해본다** — `cd web && npm run dev` 후
      `http://localhost:3000`. 우선순위: (i) 메인 페이지 상단 "내 시간표 · 친구 시간표
      보기" 버튼 → `/friends`에서 내 시간표가 실제로 보이는지, (ii) 내 시간표 + 친구 1~2명
      체크박스로 선택 → 겹쳐보기 섹션이 자동으로 나타나고 색칠/빈칸이 의도대로 보이는지,
      같은 과목을 공유로 설정해서 테스트하면 과목명이 뜨는지. 여러 세션째 브라우저 직접
      검증이 안 됐다 — 신규 기능(겹쳐보기) UI가 전혀 검증 안 된 상태라 특히 중요하다.
   b. 사용자에게 GA4 측정 ID 변경 사실을 윤서 조원에게 전달했는지 확인한다.
