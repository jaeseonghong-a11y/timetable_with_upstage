...
1. 시작 즉시 `git status --short`와 `git log --oneline -5`를 읽는다. **최우선 확인 사항**:
   a. **브라우저로 직접 열어서 클릭해본다** — `cd web && npm run dev` 후
      `http://localhost:3000`. 이번 세션에 새로 바뀐 부분 위주로: 진행단계의 2-1/2-2가
      다른 단계 원과 크기·정렬이 맞는지, step 3의 "유효 시간표 건너뛰고 AI 추천" 버튼과
      그 아래 곡선이 자연스러운지, 과목 담기 화면의 전공 선택/다른 전공 찾기 칸이 적당히
      작아졌는지, 교양과목 목록을 빠르게 스크롤할 때 체크박스가 밀리지 않는지, 필수+선택
      과목을 섞어 AI 추천을 받아 (i) 근거 문장이 필수 과목이 아니라 추가된 과목/배치를
      말하는지 (ii) 졸업요건 기여 문구가 실제로 그 후보에 담긴 교양 과목에 근거하는지
      (iii) 순위마다 근거·기여 문구가 다른지. 여러 세션째 브라우저 직접 검증이 안 됐다.
