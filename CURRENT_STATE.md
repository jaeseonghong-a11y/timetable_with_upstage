...
1. 시작 즉시 `git status --short`와 `git log --oneline -5`를 읽는다. **최우선 확인 사항**:
   a. **브라우저로 직접 열어서 클릭해본다** — `cd web && npm run dev` 후
      `http://localhost:3000`. 이번 세션에 새로 바뀐 부분 위주로: 진행단계의 2-1/2-2 연결선이
      자연스러운지(전용 선을 뺐다), 3→5 점선 곡선이 더 뚜렷하고 라벨과 안 겹치는지, 과목
      담기의 전체/전공/복수전공 칩이 가로로 흐르며 줄바꿈되는지, 2-1에서 문서 분석 후 2-2로
      갔다가 다시 2-1로 돌아와도 "다시 분석하기"가 재업로드 없이 바로 되는지, 졸업요건 목록이
      접힌 상태에서도 확인 필요 배지가 바로 보이는지, 고정 일정(알바)을 등록한 상태에서 AI
      추천을 받아 그 요일을 공강이라고 잘못 말하지 않는지. 여러 세션째 브라우저 직접 검증이
      안 됐다 — 이번엔 특히 우선순위가 높다(로직 버그 수정이 다수 포함됨).
...
3. [중간점검 후 검토] 교양/전공과목 캐싱은 인메모리 TTL이다(`web/src/lib/cache-store.ts`).
   ...
   인터페이스를 그대로 구현하는 Vercel KV/Upstash Redis로 전환한다.
4. AI 필러 후보 수 축소(현재 8개 중 5개)는 계속 보류 중 — 10000개 한도 문제가 다시 보고되면
   고려한다.
5. GitHub PR #1(`YOUNSUHPARK-patch-1`) 닫혔는지, 윤서 조원이 GA4 맞춤 정의를 등록했는지
